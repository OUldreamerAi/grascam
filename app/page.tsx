...
export default function Home() {
  ...
  const APP_REGISTRY: Record<AppType, { title: string; Content: (props?: any) => React.ReactNode }> = {
    ...
    browser:    { title: "Browser",    Content: () => <BrowserContent hideTabBar={false} /> },
  ...
  function openApp(type: AppType) {
    const windowWidth = window.innerWidth * 0.5; // w-1/2 = 50%
    const windowHeight = 550; // max-height from WindowShell
  ...
  return (
    <main className="h-screen w-screen overflow-hidden bg-cover bg-center bg-no-repeat"
      ...
      <div style={{ paddingTop: "32px", paddingLeft: "32px", display: "flex", alignItems: "flex-end", justifyContent: "center", height: fullscreenWindowIds.size > 0 ? "100vh" : "calc(100vh - 80px)" }}>
